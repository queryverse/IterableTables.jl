...
function Base.start{T,S<:NDSparse}(iter::NDSparseIterator{T,S})
    return 1
end

@generated function Base.next{T,S<:NDSparse}(iter::NDSparseIterator{T,S}, state)
...
function Base.done{T,S<:NDSparse}(iter::NDSparseIterator{T,S}, state)
